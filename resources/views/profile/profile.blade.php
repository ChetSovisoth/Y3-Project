...
@section('content')
        ...
        <div class="d-flex flex-column align-items-center mt-5 ">
            <div class="d-flex flex-row align-items-center w-100 justify-content-between">
                ...
                <div class=" ms-5 text-light flex-grow-1">
                    @livewire('follow.followers-count', ['followersCount', 'followingsCount'])
                    @livewire('follow.followings-count', ['followingsCount', 'followersCount'])
                </div>
    ...
    @livewire('follow.followers_list', ['followers', 'followings'])
    @livewire('follow.followings_list', ['followers', 'followings'])
    {{-- @livewire('follow.followers_list', ['followers', 'followings', 'followingsCount', 'followersCount'])
    @livewire('follow.followings_list', ['followers', 'followings', 'followingsCount', 'followersCount']) --}}
                    ...
                    @if ($followingsCount !== 0)
                        @foreach ($followings as $following)
                            <div class="d-flex my-3">
                                ...
                                <div class="d-flex align-items-center">
                                    @livewire('follow.is-following', ['userId' => $following->followable->id], key($following->followable->id))
                                </div>
                            </div>
                        @endforeach
                    @else
...
{{-- <script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        followingsModal.addEventListener('hide.bs.modal', function () {
            Livewire.dispatch('checkFollowingsList');
        });
    });
</script> --}}